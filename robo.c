...
void MCL_prepare(float rand_angles_gather[particle_angle_num], short rand_x_gather[MATRIX_MAP_X], short rand_y_gather[MATRIX_MAP_Y], short rand_x[P_NUMBER], short rand_y[P_NUMBER], float rand_angle[P_NUMBER]);
...
void MCL_important_sample(char *max_score_loop, short split_number[P_NUMBER], int *max_score_pos, int *max_S, float rand_angles_gather[particle_angle_num], short rand_x_gather[MATRIX_MAP_X], short rand_y_gather[MATRIX_MAP_Y], short rand_x[P_NUMBER], short rand_y[P_NUMBER], float rand_angle[P_NUMBER]);
...
//#endif
	...
	//调用函数my_MCL_particleLocalization（）得到机器人的估计位置
	//my_MCL_particleLocalization(lidar_ranges, lidar_angles, World_map, &Est_x_pose, &Est_y_pose, &Est_angle_pose, &Est_match_rate);
	my_time();
	MCL_prepare(rand_angles_gather, rand_x_gather, rand_y_gather, rand_x, rand_y, rand_angle);
...
void MCL_prepare(float rand_angles_gather[particle_angle_num], short rand_x_gather[MATRIX_MAP_X], short rand_y_gather[MATRIX_MAP_Y], short rand_x[P_NUMBER], short rand_y[P_NUMBER], float rand_angle[P_NUMBER])
{
	...
	{
		rand_x[i] = rand_x_gather[my_rand() % MATRIX_MAP_X];
		rand_y[i] = rand_y_gather[my_rand() % MATRIX_MAP_Y];
		rand_angle[i] = rand_angles_gather[my_rand() % particle_angle_num];
...
#pragma HLS pipeline II=40	 					//此循环内执行流水线操作
					...
					rand_num_temp = (my_rand() % (2 * p_move_range));
